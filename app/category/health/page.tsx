...
const HealthPage = () => {
  ...
  const healthCategories = [
    { id: 'supplements', name: 'Supplements', icon: Pill, count: 35 },
    { id: 'fitness', name: 'Fitness', icon: Heart, count: 28 }
  ]

  // Sample health products
  const healthProducts = [
    {
      id: 22,
      name: 'Multivitamin Premium Complex',
      price: 29.99,
      originalPrice: 49.99,
  ...
  const filteredAndSortedProducts = useMemo(() => {
    let filtered = healthProducts.filter(product => {
      ...
      <Container className='py-8'>
        ...
        <div className='mb-8'>
          ...
          <p className='text-lg text-gray-600 max-w-3xl'>
            Support your wellness journey with our health and fitness products. From supplements to fitness trackers, find everything for a healthier lifestyle.
          </p>
        </div>
            ...
            {healthCategories.map((subcat) => {
              ...
              return (
                <div key={subcat.id} className='bg-white rounded-lg p-4 text-center shadow-sm hover:shadow-md transition-shadow cursor-pointer border border-gray-100'>
                  <div className='inline-flex items-center justify-center w-12 h-12 rounded-lg bg-red-100 text-red-600 mb-3 mx-auto'>
